// pages/api/drive/face-search.js
// AI face search: thumbnail-first downloads + Vision API batching (16/req) + recursive subfolders
import { google } from 'googleapis';

export const config = {
  api: { bodyParser: { sizeLimit: '8mb' } },
  maxDuration: 60,
};

const MAX_DEPTH = 2;
const MAX_FOLDERS = 20;
const MAX_PHOTOS = 120;
const VISION_BATCH = 16;
const SIMILARITY_THRESHOLD = 0.68;
const IS_DEV = process.env.NODE_ENV !== 'production';

// ─── structured logger ───────────────────────────────────────────────────────────────────────────────
function log(step, data) {
  console.log(JSON.stringify({ tag: '[FACE SEARCH]', step, ...data }));
}
function logErr(step, err, extra) {
  console.error(JSON.stringify({
    tag: '[FACE SEARCH ERROR]',
    step,
    message: err?.message || String(err),
    code: err?.code || err?.status || undefined,
    stack: IS_DEV ? err?.stack : undefined,
    ...extra,
  }));
}

// ─── helpers ────────────────────────────────────────────────────────────────────────────────────────────────

async function collectFolderIds(drive, rootId) {
  const ids = [rootId];
  async function scan(folderId, depth) {
    if (depth >= MAX_DEPTH || ids.length >= MAX_FOLDERS) return;
    try {
      const res = await drive.files.list({
        q: `'${folderId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`,
        fields: 'files(id,name)',
        pageSize: 20,
      });
      const subs = res.data.files || [];
      log('subfolder_scan', { parentId: folderId, depth, found: subs.map(s => s.name) });
      await Promise.all(
        subs.map(async function (sf) {
          if (ids.length < MAX_FOLDERS) {
            ids.push(sf.id);
            await scan(sf.id, depth + 1);
          }
        })
      );
    } catch (e) {
      logErr('subfolder_scan_failed', e, { folderId, depth });
    }
  }
  await scan(rootId, 0);
  return ids;
}

function normalizeLandmarks(landmarks, box) {
  if (!landmarks || !box || !box.vertices || box.vertices.length < 3) return [];
  var x0 = box.vertices[0]?.x || 0;
  var y0 = box.vertices[0]?.y || 0;
  var x1 = box.vertices[1]?.x || 0;
  var y2 = box.vertices[2]?.y || 0;
  var w = x1 - x0;
  var h = y2 - y0;
  if (w === 0 || h === 0) return [];
  return landmarks.map(function (l) {
    return {
      type: l.type,
      x: ((l.position?.x || 0) - x0) / w,
      y: ((l.position?.y || 0) - y0) / h,
    };
  });
}

function faceSimilarity(marks1, marks2) {
  var KEY_TYPES = [
    'LEFT_EYE', 'RIGHT_EYE', 'NOSE_TIP',
    'MOUTH_LEFT', 'MOUTH_RIGHT',
    'LEFT_EYE_LEFT_CORNER', 'RIGHT_EYE_RIGHT_CORNER',
  ];
  var totalDist = 0;
  var count = 0;
  for (var i = 0; i < KEY_TYPES.length; i++) {
    var type = KEY_TYPES[i];
    var m1 = marks1.find(function (m) { return m.type === type; });
    var m2 = marks2.find(function (m) { return m.type === type; });
    if (m1 && m2) {
      totalDist += Math.sqrt(Math.pow(m1.x - m2.x, 2) + Math.pow(m1.y - m2.y, 2));
      count++;
    }
  }
  if (count === 0) return 0;
  var avgDist = totalDist / count;
  return Math.max(0, 1 - avgDist * 3);
}

async function downloadThumbnailBase64(drive, auth, fileId) {
  try {
    var meta = await drive.files.get({ fileId, fields: 'thumbnailLink,mimeType,name' });
    var thumbLink = meta.data.thumbnailLink;
    log('download_thumb_meta', { fileId, name: meta.data.name, mimeType: meta.data.mimeType, hasThumb: !!thumbLink });
    if (thumbLink) {
      var url800 = thumbLink.replace(/=s\d+$/, '=s800').replace(/=s\d+&/, '=s800&');
      if (!url800.includes('=s800')) url800 = thumbLink + '=s800';
      var client = await auth.getClient();
      var tokenRes = await client.getAccessToken();
      var token = tokenRes.token || tokenRes.res?.data?.access_token;
      if (!token) {
        log('download_thumb_no_token', { fileId });
      } else {
        var fetchRes = await fetch(url800, { headers: { Authorization: `Bearer ${token}` } });
        log('download_thumb_fetch', { fileId, status: fetchRes.status, ok: fetchRes.ok });
        if (fetchRes.ok) {
          var arrayBuf = await fetchRes.arrayBuffer();
          log('download_thumb_ok', { fileId, bytes: arrayBuf.byteLength });
          return Buffer.from(arrayBuf).toString('base64');
        } else {
          log('download_thumb_bad_status', { fileId, status: fetchRes.status });
        }
      }
    }
  } catch (e) {
    logErr('download_thumb_exception', e, { fileId });
  }

  log('download_fullres_attempt', { fileId });
  try {
    var fileRes = await drive.files.get(
      { fileId, alt: 'media' },
      { responseType: 'arraybuffer' }
    );
    var bytes = fileRes.data?.byteLength || 0;
    log('download_fullres_ok', { fileId, bytes });
    return Buffer.from(fileRes.data).toString('base64');
  } catch (e2) {
    logErr('download_fullres_exception', e2, { fileId });
    return null;
  }
}

// ─── main handler ───────────────────────────────────────────────────────────────────────────────────
export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).end();

  var eventId = req.body?.eventId;
  var selfie = req.body?.selfie;
  var selfieSize = selfie ? Math.round(selfie.length * 0.75 / 1024) : 0;

  log('request_received', { eventId, selfieKB: selfieSize, hasEventId: !!eventId, hasSelfie: !!selfie });

  if (!eventId || !selfie) {
    return res.status(400).json({ error: 'eventId and selfie required' });
  }

  var credentials;
  try {
    credentials = JSON.parse(process.env.GOOGLE_SERVICE_ACCOUNT_KEY);
    log('credentials_parsed', { clientEmail: credentials?.client_email, projectId: credentials?.project_id });
  } catch (credErr) {
    logErr('credentials_parse_failed', credErr, {});
    return res.status(500).json({
      error: 'Server configuration error: invalid service account key',
      step: 'credentials_parse',
      detail: IS_DEV ? credErr.message : undefined,
    });
  }

  var auth;
  try {
    auth = new google.auth.GoogleAuth({
      credentials,
      scopes: [
        'https://www.googleapis.com/auth/drive.readonly',
        'https://www.googleapis.com/auth/cloud-platform',
      ],
    });
    var testClient = await auth.getClient();
    var testToken = await testClient.getAccessToken();
    log('auth_ok', { hasToken: !!testToken?.token });
  } catch (authErr) {
    logErr('auth_failed', authErr, {});
    return res.status(500).json({
      error: 'Authentication failed',
      step: 'google_auth',
      detail: IS_DEV ? authErr.message : undefined,
    });
  }

  log('selfie_detection_start', {});
  var vision = google.vision({ version: 'v1', auth });
  var selfieBase64 = selfie.replace(/^data:image\/\w+;base64,/, '');
  log('selfie_base64_length', { chars: selfieBase64.length });

  var selfieFaces;
  try {
    var selfieVision = await vision.images.annotate({
      requestBody: {
        requests: [{
          image: { content: selfieBase64 },
          features: [{ type: 'FACE_DETECTION', maxResults: 5 }],
        }],
      },
    });
    log('selfie_vision_raw_response', {
      responses: selfieVision.data.responses?.length,
      error: selfieVision.data.responses?.[0]?.error || null,
    });
    selfieFaces = selfieVision.data.responses?.[0]?.faceAnnotations || [];
    log('selfie_faces_detected', { count: selfieFaces.length });
  } catch (visErr) {
    logErr('selfie_vision_api_failed', visErr, {});
    return res.status(500).json({
      error: 'Vision API failed on selfie',
      step: 'selfie_face_detection',
      detail: IS_DEV ? visErr.message : undefined,
    });
  }

  if (selfieFaces.length === 0) {
    log('selfie_no_face', {});
    return res.status(400).json({
      error: 'ไม่พบใบหน้าในรูปที่อัปโหลด กรุณาใช้รูปที่เห็นใบหน้าชัดเจน',
    });
  }

  var selfieMarks = normalizeLandmarks(selfieFaces[0].landmarks, selfieFaces[0].boundingPoly);
  log('selfie_landmarks_normalized', { landmarkCount: selfieMarks.length });

  log('folder_scan_start', { eventId });
  var drive = google.drive({ version: 'v3', auth });
  var folderIds;
  try {
    folderIds = await collectFolderIds(drive, eventId);
    log('folder_scan_done', { folderCount: folderIds.length, folderIds });
  } catch (folderErr) {
    logErr('folder_scan_failed', folderErr, { eventId });
    return res.status(500).json({
      error: 'Failed to scan event folders',
      step: 'folder_scan',
      detail: IS_DEV ? folderErr.message : undefined,
    });
  }

  log('photo_list_start', { folderCount: folderIds.length });
  var photos;
  try {
    var folderQuery = folderIds.map(function (id) { return `'${id}' in parents`; }).join(' or ');
    var q = `(${folderQuery}) and mimeType contains 'image/' and not name contains 'Poster_info' and trashed=false`;
    log('photo_list_query', { q });
    var photosRes = await drive.files.list({
      q,
      fields: 'files(id,name,mimeType)',
      pageSize: MAX_PHOTOS,
      orderBy: 'name',
    });
    photos = photosRes.data.files || [];
    log('photo_list_done', {
      count: photos.length,
      sample: photos.slice(0, 3).map(p => ({ id: p.id, name: p.name, mimeType: p.mimeType })),
    });
  } catch (listErr) {
    logErr('photo_list_failed', listErr, {});
    return res.status(500).json({
      error: 'Failed to list photos from Drive',
      step: 'photo_list',
      detail: IS_DEV ? listErr.message : undefined,
    });
  }

  if (photos.length === 0) {
    log('no_photos_found', {});
    return res.status(200).json({ matches: [], total: 0, searched: 0 });
  }

  var matches = [];
  var totalBatches = Math.ceil(photos.length / VISION_BATCH);

  for (var i = 0; i < photos.length; i += VISION_BATCH) {
    var batchNum = Math.floor(i / VISION_BATCH) + 1;
    var batch = photos.slice(i, i + VISION_BATCH);
    log('batch_start', { batchNum, total: totalBatches, batchSize: batch.length });

    var base64Results;
    try {
      base64Results = await Promise.allSettled(
        batch.map(function (photo) {
          return downloadThumbnailBase64(drive, auth, photo.id);
        })
      );
    } catch (dlErr) {
      logErr('batch_download_unexpected', dlErr, { batchNum });
      continue;
    }

    var successCount = base64Results.filter(r => r.status === 'fulfilled' && r.value).length;
    var failCount = base64Results.length - successCount;
    log('batch_download_done', { batchNum, successCount, failCount });

    var visionRequests = [];
    var batchIndex = [];
    for (var j = 0; j < base64Results.length; j++) {
      var b64 = base64Results[j].status === 'fulfilled' ? base64Results[j].value : null;
      if (b64) {
        visionRequests.push({
          image: { content: b64 },
          features: [{ type: 'FACE_DETECTION', maxResults: 10 }],
        });
        batchIndex.push(j);
      }
    }

    if (visionRequests.length === 0) {
      log('batch_no_valid_images', { batchNum });
      continue;
    }

    log('vision_batch_request', { batchNum, requestCount: visionRequests.length });

    try {
      var visionRes = await vision.images.annotate({
        requestBody: { requests: visionRequests },
      });
      var responses = visionRes.data.responses || [];
      log('vision_batch_response', {
        batchNum,
        responseCount: responses.length,
        errors: responses.filter(r => r.error).map(r => r.error),
      });

      for (var r = 0; r < responses.length; r++) {
        var photoIdx = batchIndex[r];
        var photo = batch[photoIdx];
        var respErr = responses[r].error;
        if (respErr) {
          log('vision_response_error', { batchNum, r, fileId: photo.id, error: respErr });
          continue;
        }
        var faces = responses[r].faceAnnotations || [];
        var bestScore = 0;
        for (var f = 0; f < faces.length; f++) {
          var marks = normalizeLandmarks(faces[f].landmarks, faces[f].boundingPoly);
          var score = faceSimilarity(selfieMarks, marks);
          if (score > bestScore) bestScore = score;
        }
        log('face_comparison', { fileId: photo.id, name: photo.name, facesDetected: faces.length, bestScore: bestScore.toFixed(3) });
        if (bestScore >= SIMILARITY_THRESHOLD) {
          matches.push({
            id: photo.id,
            name: photo.name,
            score: bestScore,
            thumbnailUrl: `/api/drive/photo?fileId=${photo.id}&size=thumb`,
            downloadUrl: `/api/drive/photo?fileId=${photo.id}`,
          });
          log('match_found', { fileId: photo.id, name: photo.name, score: bestScore.toFixed(3) });
        }
      }
    } catch (vErr) {
      logErr('vision_batch_api_failed', vErr, { batchNum, requestCount: visionRequests.length });
    }

    log('batch_done', { batchNum, matchesSoFar: matches.length });
  }

  matches.sort(function (a, b) { return b.score - a.score; });
  log('search_complete', { totalMatches: matches.length, searched: photos.length });

  return res.status(200).json({
    matches,
    total: matches.length,
    searched: photos.length,
  });
                               }
