...
function uploadToGoogle(file, uploadUrl, onProgress) {
  return new Promise(function (resolve, reject) {
    ...
    xhr.onload = function () {
      ...
      else reject(new Error('Upload error: HTTP ' + xhr.status));
    };
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)', marginBottom: 12 }}>
      ...
      {error && (
        <div style={{ background: 'rgba(255,80,80,0.15)', border: '1px solid rgba(255,80,80,0.3)', borderRadius: 8, padding: '10px 20px', fontSize: 13, color: '#ff6b6b' }}>
          {error === 'no_code' ? 'ยกเลิกการเข้าสู่ระบบ' : 'เกิดข้อผิดพลาด: ' + error}
        </div>
      )}
      <a
        href="/api/auth/login"
      ...
      <a href="/" style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', textDecoration: 'none', marginTop: 8 }}>
        ← กลับหน้าหลัก
      </a>
    </div>
  );
...
function UploaderPanel({ user }) {
  ...
  useEffect(function () {
    fetch('/api/drive/browse')
      ...
      .then(function (data) {
        var f = (data.folders || []).map(function (fo) { return { id: fo.id, name: fo.name }; });
        setFolders(f);
  ...
  function addFiles(newFiles) {
    var items = Array.from(newFiles).map(function (f) {
      return { file: f, progress: 0, status: 'pending', error: null };
    });
    setFileQueue(function (prev) { return prev.concat(items); });
    setAllDone(false);
  }
  ...
  async function createFolder() {
    ...
    try {
      var r = await fetch('/api/drive/mkdir', {
        method: 'POST',
      ...
      if (data.id) {
        var newF = { id: data.id, name: data.name };
        setFolders(function (prev) { return [newF].concat(prev); });
        setTargetFolder(newF);
        setNewFolderName('');
  ...
  async function startUpload() {
    ...
    for (var i = 0; i < fileQueue.length; i++) {
      ...
      // Mark as uploading
      (function (idx) {
        setFileQueue(function (prev) {
          return prev.map(function (f, j) {
            return j === idx ? Object.assign({}, f, { status: 'uploading', progress: 0 }) : f;
          });
        });
      })(i);

      try {
        // Get resumable upload URL from server
        var startRes = await fetch('/api/drive/start-upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            name: item.file.name,
        ...
        // Upload file directly to Google with progress
        var idx = i;
        await uploadToGoogle(item.file, startData.uploadUrl, function (pct) {
          setFileQueue(function (prev) {
            return prev.map(function (f, j) {
              return j === idx ? Object.assign({}, f, { progress: pct }) : f;
            });
          });
        });
        ...
        (function (idx2) {
          setFileQueue(function (prev) {
            return prev.map(function (f, j) {
              return j === idx2 ? Object.assign({}, f, { status: 'done', progress: 100 }) : f;
            });
          });
        })(i);
      } catch (e) {
        (function (idx3, err) {
          setFileQueue(function (prev) {
            return prev.map(function (f, j) {
              return j === idx3 ? Object.assign({}, f, { status: 'error', error: err.message }) : f;
            });
          });
        })(i, e);
      }
  ...
  return (
    <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 20px', fontFamily: FONT, color: '#fff' }}>
      ...
      <Section title="โฟลเดอร์ปลายทาง">
        {loadingFolders ? (
          <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>กำลังโหลด...</div>
        ) : (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            <FolderChip
              label="อัลบั้ม (Root)"
              selected={!targetFolder}
              onClick={function () { setTargetFolder(null); }}
            />
            {folders.map(function (f) {
              return (
                <FolderChip
                  key={f.id}
                  label={f.name}
                  selected={targetFolder && targetFolder.id === f.id}
                  onClick={function () { setTargetFolder(f); }}
                />
              );
            })}
            <button
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12, alignItems: 'center' }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: creatingFolder ? 'default' : 'pointer', fontFamily: FONT,
              }}
            >
              {creatingFolder ? '...' : 'สร้าง'}
            </button>
          </div>
        )}
        ...
        {targetFolder && (
          <div style={{ marginTop: 10, fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>
            อัปโหลดไปที่: <span style={{ color: 'rgba(255,255,255,0.7)' }}>{targetFolder.name}</span>
          </div>
        )}
      </Section>
      ...
      <Section title="เลือกภาพ">
        <div
          ...
          style={{
            border: '2px dashed ' + (isDragging ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.18)'),
            borderRadius: 12,
            padding: '40px 20px',
            textAlign: 'center',
            cursor: 'pointer',
        ...
        >
          ...
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginTop: 4 }}>
            หรือคลิกเพื่อเลือกไฟล์ · JPG, PNG, RAW, MP4
          </div>
        </div>
        <input
          ref={fileInputRef}
          type="file"
          multiple
          accept="image/*,video/*,.raw,.cr2,.cr3,.nef,.arw,.dng"
          style={{ display: 'none' }}
      ...
      {fileQueue.length > 0 && (
        <Section title={'ไฟล์ที่เลือก (' + fileQueue.length + ' ไฟล์)'}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {fileQueue.map(function (item, idx) {
              return (
                <FileRow
                  key={idx}
                  item={item}
                  onRemove={item.status === 'pending' ? function () { removeFile(idx); } : null}
                />
              );
            })}
          </div>

          {allDone && doneCount === fileQueue.length && (
            <div style={{
              marginTop: 16, padding: '12px 20px', borderRadius: 10,
              background: 'rgba(72,199,142,0.15)', border: '1px solid rgba(72,199,142,0.3)',
              color: '#48c78e', fontSize: 13, textAlign: 'center',
            }}>
              ✓ อัปโหลดสำเร็จทั้งหมด {doneCount} ไฟล์
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button
              ...
              style={{
                ...
                border: '1px solid rgba(255,255,255,0.2)',
                color: pendingCount > 0 && !isUploading ? '#fff' : 'rgba(255,255,255,0.3)',
                fontSize: 14, cursor: pendingCount > 0 && !isUploading ? 'pointer' : 'default',
...
function chipStyle(selected) {
  return {
    ...
    background: selected ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)',
    border: '1px solid ' + (selected ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.15)'),
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {item.status === 'uploading' && (
          <div style={{
            height: 2, background: 'rgba(255,255,255,0.1)', borderRadius: 1, marginTop: 6,
          }}>
            <div style={{
              height: '100%', width: item.progress + '%',
              background: '#60a5fa', borderRadius: 1,
              transition: 'width 0.3s ease',
            }} />
          </div>
        )}
        {item.status === 'error' && (
          <div style={{ fontSize: 11, color: '#f87171', marginTop: 2 }}>{item.error}</div>
        )}
      </div>
      <div style={{ fontSize: 12, color: statusColor, minWidth: 28, textAlign: 'right' }}>
        {statusLabel}
      </div>
      {onRemove && (
        <button
...
export default function Uploader() {
  var [user, setUser] = useState(null); // null=loading, false=not authed, {...}=authed
  ...
  useEffect(function () {
    ...
    fetch('/api/auth/me')
      ...
      .then(function (data) { setUser(data || false); })
      .catch(function () { setUser(false); });
  }, []);
    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh' }}>
      ...
      {user === null && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 52px)' }}>
          <div style={{ width: 32, height: 32, border: '2px solid rgba(255,255,255,0.1)', borderTopColor: 'rgba(255,255,255,0.5)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        </div>
      )}
